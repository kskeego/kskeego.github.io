...
// Auto-injects a single cookie banner (hidden on first load by default-accept),
// stores consent, gates scripts, and forwards to major vendors if present.
(function () {
  const KEY = 'consent_v1';
  const TTL_DAYS = 180;

  // Build banner + styles in DOM so you don't have to edit every page.
...
.cc-close{position:absolute;top:.5rem;right:.5rem;background:transparent;border:0;color:#fff;font-size:1.25rem;cursor:pointer}`;
  ...
  // Vendor adapters (presence-detected)
  const adapters = {
    ...
    onetrust: {
      present: () => !!document.getElementById('onetrust-banner-sdk') || !!document.getElementById('onetrust-consent-sdk'),
      apply: (s) => {
    ...
    cookieyes: {
      present: () => !!document.querySelector('.cky-consent-container, #cookieyes'),
      apply: (s) => {
  ...
  function forwardToVendors(state){
    Object.values(adapters).forEach(ad=>{
      try{ if(ad.present()) ad.apply(state) }catch(e){ /* silent */ }
    });
  }
  ...
  function bootUI(prev){
    ...
    btnAccept?.addEventListener('click', () => {
      applyAndClose({ essential:true, analytics:true, marketing:true, ts:Date.now() });
    });
    btnReject?.addEventListener('click', () => {
      applyAndClose({ essential:true, analytics:false, marketing:false, ts:Date.now() });
    });
    btnSave?.addEventListener('click', () => {
      applyAndClose({
        essential:true,
        analytics: !!elAn?.checked,
        marketing: !!elMk?.checked,
        ts:Date.now()
      });
    });
    ...
    if (prev){
      ...
      // Default accept on first visit and keep banner hidden.
      ...
      banner.hidden = true; // keep hidden unless you add a "Privacy settings" link to reopen
    }
  }
